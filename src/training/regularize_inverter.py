...
with tqdm(total=len(loader.dataset), unit='img') as pbar:
    for ori_img, ori_label in loader:
        ...
        code, results, results_vis, log_message = util_inverter.train_z(general_reports_dir=general_reports_dir, source_interim_dir=source_interim_dir, iid_classes=iid_class, plot_training_dir=plot_training_dir,
                                                                        x=ori_img, generator=G, encoder=E, device=device, early_stopping=50)
        ...
        util_report_inverter.save_image(os.path.join(general_reports_dir, 'ori.png'), util_inverter.postprocess(
            util_inverter.get_tensor_value(ori_img))[0][0])
