...
try:
    model = util_cnn.get_cnn(model_name=cfg['model_cnn']['model_name'], image_channels=channel, iid_classes=iid_class, n_class=len(iid_class), img_dim=image_size, cnn_args=cfg['model_cnn']['network'])
    model.load_state_dict(torch.load(os.path.join(model_dir, f"model.pt"), map_location=torch.device(device)))
    model = model.to(device)
except FileNotFoundError:
    # Architecture
    print("Model architecture")
    model = util_cnn.get_cnn(model_name=cfg['model_cnn']['model_name'], image_channels=cfg['data']['channel'], iid_classes=iid_class,  n_class=len(iid_class),  img_dim=cfg['data']['image_size'], cnn_args=cfg['model_cnn']['network'])
    model=model.to(device)
    ...
    util_general.notification_ifttt(info=f"cnn training started")
    tik = time.time()
    model, history = util_cnn.train_model_multipatient(
        ...
        early_stopping=cfg['trainer_cnn']['early_stopping'], model_dir=model_dir, device=device)
    tok = time.time()
    ...
    #todo aggiungi evaluation finale come metriche per classe
