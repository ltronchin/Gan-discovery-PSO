...
# Data loaders
print("\nCreate iid dataloader")
iid_train_dataset, _ = util_data.get_public_dataset_inverter(dataset_name=dataset_name, data_dir=data_dir, drange_net=cfg['data']['drange_net'], general_reports_dir=general_reports_dir, image_size=image_size, channel=channel, iid_class=iid_class)
# Define the dataloader
iid_data_loader = DataLoader(dataset=iid_train_dataset, batch_size=cfg['trainer_ae']['batch_size'], shuffle=True, drop_last=False)
# Check on the data
print("Sanity check on data")
for dset, loader in zip(['iid'], [iid_data_loader]):
...
for label in iid_class:
    ...
    idx_batch = 0
    with tqdm(total=len(iid_train_dataset), unit='img') as pbar:
        for x_batch, y_batch in iid_data_loader:
            ...
            y_mask = y_batch == label # select only the iid_class of interest
            ...
            if idx_batch == 0:
                latent_iid_class = latent_batch
            else:
                latent_iid_class = np.concatenate((latent_iid_class, latent_batch), axis=0)

            pbar.update(x_batch.shape[0])
            idx_batch += 1

    # Dataframe: iid_class
    # row -> n_images of iid_class
    # column -> feature
    df = pd.DataFrame(latent_iid_class)
